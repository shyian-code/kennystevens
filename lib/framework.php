...
/**
 * Various functions required for kennystevens-foundation to work properly
 *
 *
 */
/*
 ...
 */
if ( ! class_exists( 'kennystevens_navwalker' ) ) :
	class kennystevens_navwalker extends Walker_Nav_Menu {
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			$indent = str_repeat("\t", $depth);
			$output .= "\n$indent<ul class=\"dropdown menu vertical\" data-toggle>\n";
		}
	}
	if ( ! class_exists( 'kennystevens_mobile_navwalker' ) ) :
		class kennystevens_mobile_navwalker extends Walker_Nav_Menu {
			function start_lvl( &$output, $depth = 0, $args = array() ) {
				$indent = str_repeat("\t", $depth);
 ...
 */
if ( ! function_exists( 'kennystevens_menu_fallback' ) ) :
	function kennystevens_menu_fallback() {
...
if ( ! function_exists( 'kennystevens_active_nav_class' ) ) :
	function kennystevens_active_nav_class( $classes, $item ) {
		if ( 1 == $item->current || true == $item->current_item_ancestor ) {
			$classes[] = 'active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'kennystevens_active_nav_class', 10, 2 );
endif;

// 3 - Pagination: {@link https://codex.wordpress.org/Pagination}

if ( ! function_exists( 'kennystevens_pagination' ) ) :
	function kennystevens_pagination() {
		global $wp_query;
 ...
 */
if ( ! function_exists( 'kennystevens_active_list_pages_class' ) ) :
	function kennystevens_active_list_pages_class( $input ) {
		$pattern = '/current_page_item/';
		$replace = 'current_page_item active';
		$output = preg_replace( $pattern, $replace, $input );
		return $output;
	}
	add_filter( 'wp_list_pages', 'kennystevens_active_list_pages_class', 10, 2 );
endif;

// 4 - Comments tree: @link {https://codex.wordpress.org/Function_Reference/Walker_Comment}

if ( ! class_exists( 'kennystevens_Comments' ) ) :
	class kennystevens_Comments extends Walker_Comment{
